...
# cympy.study.New()
# test_study = r'V:/z_JessePiland/CPAT/Anderson-Clemson_test/Anderson_Clemson-1.sxst'
# # test_study = r'V:/z_JessePiland/CPAT/CPAT_test_study/Byrum_Creek.xst'
# cympy.study.Open(test_study)

### Check if study is self-contained and establish output path
studypath = cympy.study.info.Path
if studypath == '':
    desktop = os.path.join(os.environ['USERPROFILE'],'Desktop')
    reportpath = desktop
else:
    reportpath = studypath.rsplit('\\',1)[0].rsplit('/',1)[0]

print(f'Reports will be saved in the following location: {reportpath}')
if connect_dg:
    resultlocation = reportpath+'\CPAT_Output_DG_Connected\\'
elif disconnect_dg:
    resultlocation = reportpath+'\CPAT_Output_DG_Disconnected\\'
else:
    resultlocation = reportpath+'\CPAT_Output_DG_As_Is\\'
...
print('Feeder breaker relay settings saved to ' + resultlocation)
...
print('CYME device report saved to ' + resultlocation)
